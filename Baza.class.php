...
class Baza {
    ...
    const korisnik = "WebDiP2017x080";
    const lozinka = "admin_TZYH";
    const baza = "WebDiP2017x080";
    ...
    function DohvatiVrijemePomaka(){
        $rezultat= $this->veza->query("SELECT pomak FROM konfiguracija_sustava WHERE id=(SELECT MAX(id) FROM konfiguracija_sustava)");
        $pomak= $rezultat->fetch_array();
        
        return date ("Y-m-d H:i:s",($pomak[0]*60*60)+ time());
    }
